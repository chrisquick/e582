...
if "cleanall" in args:
    ...
    the_files=glob.glob('*.so') + ['fastbinit.cpp','fasthist.cpp','bitmap.cpp']
...
fasthist=Extension('fasthist', ['fasthist.pyx','histo.cpp'],
                       include_dirs = [numpy.get_include()],
                       extra_compile_args=['-O3', '-fPIC'],
                       library_dirs=['.'],
                       language="c++")

bitmask=Extension('bitmap', ['bitmap.pyx','bitmask.cpp'],
                       include_dirs = [numpy.get_include(),'.'],
                       extra_compile_args=['-O3', '-fPIC'],
                       library_dirs=['.'],
                       language="c++")
...
ext_modules = [fastbin,fasthist,bitmask]
